...
/// the chip.
pub fn generate_permutation_trace<F: PrimeField, EF4: ExtensionField<F>, SepticExtension: ExtensionField<F>>(
    sends: &[Interaction<F>],
    receives: &[Interaction<F>],
    ...
    random_elements: &[EF4],
    batch_size: usize,
) -> (RowMajorMatrix<F>, EF4, SepticExtension) {
    let empty = vec![];
        ...
        Some(prep) => {
            permutation_trace
                ...
                .for_each(|((row, prep_row), main_row)| {
                    ...
                    populate_global_permutation_row::<F, SepticExtension>(
                        global_row,
                        prep_row,
                        main_row,
                        global_sends,
    ...
    //         } else {
    //             let row = &row[(global_row_range.end - SepticExtension::D * 2)..global_row_range.end];
    //             SepticExtension::from_base_slice(row)
    //         }
    //     })
    //     .collect::<Vec<_>>();
    ...
    (permutation_trace, EF4::ZERO, SepticExtension::ZERO)
}
