...
impl CostEstimator for ExecutionReport {
    fn estimate_area(&self) -> u64 {
        ...
        total_area += (sha_extend_events as u64) * costs[&MipsAirDiscriminants::Sha256Extend];
        total_chips += 1;

        let sha_compress_events = self.syscall_counts[SyscallCode::SHA_COMPRESS];
        total_area += (sha_compress_events as u64) * costs[&MipsAirDiscriminants::Sha256Compress];
        total_chips += 1;

        let ed_add_events = self.syscall_counts[SyscallCode::ED_ADD];
        total_area += (ed_add_events as u64) * costs[&MipsAirDiscriminants::Ed25519Add];
        total_chips += 1;

        let ed_decompress_events = self.syscall_counts[SyscallCode::ED_DECOMPRESS];
        total_area +=
            (ed_decompress_events as u64) * costs[&MipsAirDiscriminants::Ed25519Decompress];
        total_chips += 1;

        let k256_decompress_events = self.syscall_counts[SyscallCode::SECP256K1_DECOMPRESS];
        total_area +=
            (k256_decompress_events as u64) * costs[&MipsAirDiscriminants::K256Decompress];
        total_chips += 1;

        let p256_decompress_events = self.syscall_counts[SyscallCode::SECP256R1_DECOMPRESS];
        total_area +=
            (p256_decompress_events as u64) * costs[&MipsAirDiscriminants::P256Decompress];
        total_chips += 1;

        let secp256k1_add_events = self.syscall_counts[SyscallCode::SECP256K1_ADD];
        total_area += (secp256k1_add_events as u64) * costs[&MipsAirDiscriminants::Secp256k1Add];
        total_chips += 1;

        let secp256k1_double_events = self.syscall_counts[SyscallCode::SECP256K1_DOUBLE];
        total_area +=
            (secp256k1_double_events as u64) * costs[&MipsAirDiscriminants::Secp256k1Double];
        total_chips += 1;

        let secp256r1_add_events = self.syscall_counts[SyscallCode::SECP256R1_ADD];
        total_area += (secp256r1_add_events as u64) * costs[&MipsAirDiscriminants::Secp256r1Add];
        total_chips += 1;

        let secp256r1_double_events = self.syscall_counts[SyscallCode::SECP256R1_DOUBLE];
        total_area +=
            (secp256r1_double_events as u64) * costs[&MipsAirDiscriminants::Secp256r1Double];
        total_chips += 1;

        let keccak256_permute_events = self.syscall_counts[SyscallCode::KECCAK_PERMUTE];
        total_area += (keccak256_permute_events as u64) * costs[&MipsAirDiscriminants::KeccakP];
        total_chips += 1;

        let bn254_add_events = self.syscall_counts[SyscallCode::BN254_ADD];
        total_area += (bn254_add_events as u64) * costs[&MipsAirDiscriminants::Bn254Add];
        total_chips += 1;

        let bn254_double_events = self.syscall_counts[SyscallCode::BN254_DOUBLE];
        total_area += (bn254_double_events as u64) * costs[&MipsAirDiscriminants::Bn254Double];
        total_chips += 1;

        let bls12381_add_events = self.syscall_counts[SyscallCode::BLS12381_ADD];
        total_area += (bls12381_add_events as u64) * costs[&MipsAirDiscriminants::Bls12381Add];
        total_chips += 1;

        let bls12381_double_events = self.syscall_counts[SyscallCode::BLS12381_DOUBLE];
        total_area +=
            (bls12381_double_events as u64) * costs[&MipsAirDiscriminants::Bls12381Double];
        total_chips += 1;

        let uint256_mul_events = self.syscall_counts[SyscallCode::UINT256_MUL];
        total_area += (uint256_mul_events as u64) * costs[&MipsAirDiscriminants::Uint256Mul];
        total_chips += 1;

        let u256xu2048_mul_events = self.syscall_counts[SyscallCode::U256XU2048_MUL];
        total_area += (u256xu2048_mul_events as u64) * costs[&MipsAirDiscriminants::U256x2048Mul];
        total_chips += 1;
        ...
        total_area += (bls12381_fp_events as u64) * costs[&MipsAirDiscriminants::Bls12381Fp];
        total_chips += 1;
        ...
        total_area +=
            (bls12381_fp2_addsub_events as u64) * costs[&MipsAirDiscriminants::Bls12381Fp2AddSub];
        total_chips += 1;

        let bls12381_fp2_mul_events = self.syscall_counts[SyscallCode::BLS12381_FP2_MUL];
        total_area +=
            (bls12381_fp2_mul_events as u64) * costs[&MipsAirDiscriminants::Bls12381Fp2Mul];
        total_chips += 1;
        ...
        total_area += (bn254_fp_events as u64) * costs[&MipsAirDiscriminants::Bn254Fp];
        total_chips += 1;

        let bn254_fp2_addsub_events = self.syscall_counts[SyscallCode::BN254_FP2_ADD]
            + self.syscall_counts[SyscallCode::BN254_FP2_SUB];
        total_area +=
            (bn254_fp2_addsub_events as u64) * costs[&MipsAirDiscriminants::Bn254Fp2AddSub];
        total_chips += 1;

        let bn254_fp2_mul_events = self.syscall_counts[SyscallCode::BN254_FP2_MUL];
        total_area += (bn254_fp2_mul_events as u64) * costs[&MipsAirDiscriminants::Bn254Fp2Mul];
        total_chips += 1;

        let bls12381_decompress_events = self.syscall_counts[SyscallCode::BLS12381_DECOMPRESS];
        total_area +=
            (bls12381_decompress_events as u64) * costs[&MipsAirDiscriminants::Bls12381Decompress];
        total_chips += 1;

        let syscall_events = self.syscall_counts.values().sum::<u64>();
        total_area += (syscall_events as u64) * costs[&MipsAirDiscriminants::SyscallCore];
        total_chips += 1;
        ...
        total_area += (addsub_events as u64) * costs[&MipsAirDiscriminants::Add];
        total_chips += 1;

        let bitwise_events = self.opcode_counts[Opcode::AND]
            + self.opcode_counts[Opcode::OR]
            + self.opcode_counts[Opcode::XOR];
        total_area += (bitwise_events as u64) * costs[&MipsAirDiscriminants::Bitwise];
        total_chips += 1;
        ...
        total_area += (shift_left_events as u64) * costs[&MipsAirDiscriminants::ShiftLeft];
        total_chips += 1;
        ...
        total_area += (memory_global_finalize_events as u64)
            * costs[&MipsAirDiscriminants::MemoryGlobalFinal];
        total_chips += 1;

        let memory_local_initialize_events = self.touched_memory_addresses;
        total_area +=
            (memory_local_initialize_events as u64) * costs[&MipsAirDiscriminants::MemoryLocal];
        total_chips += 1;
