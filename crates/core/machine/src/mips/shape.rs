...
pub struct CoreShapeConfig<F: PrimeField32> {
    ...
    allowed_preprocessed_log_heights: HashMap<MipsAir<F>, Vec<Option<usize>>>,
    allowed_core_log_heights: Vec<HashMap<MipsAir<F>, Vec<Option<usize>>>>,
    maximal_core_log_heights_mask: Vec<bool>,
    memory_allowed_log_heights: HashMap<MipsAir<F>, Vec<Option<usize>>>,
    precompile_allowed_log_heights: HashMap<MipsAir<F>, (usize, Vec<usize>)>,
}
...
pub mod tests {
    ...
    fn test_dummy_record() {
        ...
        let core_log_heights = [
            ...
            (MipsAir::<KoalaBear>::Global(GlobalChip), 10),
        ];
