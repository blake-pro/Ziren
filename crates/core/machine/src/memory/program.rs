...
impl<F: PrimeField32> MachineAir<F> for MemoryProgramChip {
    ...
    fn generate_preprocessed_trace(&self, program: &Self::Program) -> Option<RowMajorMatrix<F>> {
        ...
        let memory = program.image.iter().sorted().collect::<Vec<_>>();
        values
...
where
    AB: ZKMAirBuilder + PairBuilder + AirBuilderWithPublicValues,
{
    fn eval(&self, builder: &mut AB) {
        ...
        let public_values_slice: [AB::Expr; ZKM_PROOF_NUM_PV_ELTS] =
